...
bool CCoinsView::BatchWrite(CCoinsMap &mapCoins, const uint256 &hashBlock, unsigned char mode) { return false; }
...
bool CCoinsViewBacked::BatchWrite(CCoinsMap &mapCoins, const uint256 &hashBlock, unsigned char mode) { return base->BatchWrite(mapCoins, hashBlock, mode); }
...
CCoinsMap::const_iterator CCoinsViewCache::FetchCoins(const uint256 &txid) const {
    ...
    size_t usage = ret->second.coins.DynamicMemoryUsage();
    cachedCoinsUsage += usage;
    if (usage > CCoins::LARGE_COINS)
        ret->second.flags |= CCoinsCacheEntry::BIG;
    return ret;
}
...
bool CCoinsViewCache::Flush() {
    bool fOk = base->BatchWrite(cacheCoins, hashBlock, CCoinsView::NORMAL);
    cacheCoins.clear();
    cachedCoinsUsage = 0;
    return fOk;
}
...
bool CCoinsViewCache::HotFlush(unsigned char mode) {
    bool fOk = base->BatchWrite(cacheCoins, hashBlock, mode);
    //Recalculate cachedCoinsUsage
    cachedCoinsUsage = 0;
...
CCoinsModifier::~CCoinsModifier()
{
    ...
    if ((it->second.flags & CCoinsCacheEntry::FRESH) && it->second.coins.IsPruned()) {
        ...
        size_t usage = it->second.coins.DynamicMemoryUsage();
        cache.cachedCoinsUsage += usage;
        if (usage > CCoins::LARGE_COINS)
            it->second.flags |= CCoinsCacheEntry::BIG;
        else
            it->second.flags &= ~CCoinsCacheEntry::BIG;
    }
}
