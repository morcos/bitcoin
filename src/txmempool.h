...
struct LockPoints
{
    ...
    // time constraints given our view of block chain history maxed with any
    // nLockTime requirement
    int height;
    int64_t time;
    // Preceding height and time might have been calculated using relatively
    // locked inputs.  As long as the current chain descends from the highest
    // block containing one of these inputs the calculation is still valid.
    uint256 maxInputHash;

    LockPoints() : height(0), time(0), maxInputHash() { }
};
...
{
    update_lock_points(LockPoints _lp) : lp(_lp) { }
