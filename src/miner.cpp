...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool validate)
{
    resetBlock();
...
void BlockAssembler::addPackageTxs(bool failFast)
{
    ...
    while (mi != mempool.mapTx.get<ancestor_score>().end() || !mapModifiedTx.empty())
    {
        ...
        if (failFast && nBlockWeight >= ((nBlockMaxWeight / 8) * std::min((size_t)64, (nCoinCacheUsage >> 25))))
            return;
