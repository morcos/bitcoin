...
static bool EvaluateSequenceLocks(int flags, const CBlockIndex& block, std::pair<int, int64_t> lockPair)
{
...
{
    return EvaluateSequenceLocks(flags, block, CalculateSequenceLocks(tx, flags, prevHeights, block));
}
...
bool CheckSequenceLocks(const CTransaction &tx, int flags)
{
    ...
    return EvaluateSequenceLocks(flags, index, lockPair);
}
