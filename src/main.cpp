...
CCoinsViewUndoCache *pcoinsTip = NULL;
...
static void WarmTipCache(const CChainParams& chainparams)
{
    ...
    std::unique_ptr<CBlockTemplate> blocktemplate(assembler.CreateNewBlock(CScript(), false));
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode) {
    ...
    bool fTrimFlush = mode == FLUSH_STATE_PERIODIC && !fDoFullFlush && cacheSize * (10.0/6) > nCoinCacheUsage && nNow > nLastTrim + (int64_t)DATABASE_TRIM_INTERVAL * 1000000; 
