...
class EstimateFeeTest(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        split_inputs_by(self.nodes[1:], 3, self.nodes[0])
        print("Finished splitting")

        stop_node(self.nodes[0],0)
         
        # now replace nodes[0] with node3 without having polluted the estimates
        self.nodes[0] = start_node(3, self.options.tmpdir,
                              ["-debug=estimatefee"])
        connect_nodes(self.nodes[0], 1)  #reconnect now that we removed the one they were connected too
    ...
    def run_test(self):
        ...
        for i in range(len(self.nodes)):
            utxo = self.nodes[i].listunspent(1)
            random.shuffle(utxo)
            inputlists.append(utxo)
            to_addresses.append(self.nodes[i].getnewaddress())
            from_addresses.append(self.nodes[i].getnewaddress())
        ...
        for i in range(1500):
            ...
            (txid, newinputlist, txhex, fee) = small_tx_from_inputs_randfee(self.nodes[from_index], from_addresses[from_index],
